...
const NewMessage = () => {
   ...
   return (
      <div className={styles["tabs-list"]}>
         ...
         {notifications && showLast && (
            <span className={styles["arrrow-down"]} onClick={goDown}>
               <img src="assets/images/icons/down-arrow.png" alt="" />
            </span>
         )}
         <div className={styles["new-message-list"]}>
            <ul>
               {allNotifications && notifications && width >= 992
                  ? notifications?.map((messageNew) => (
                       <li key={messageNew?.messageid}>
                          <div
                             className={styles["list-item"]}
                             onClick={() => ReadMsg(messageNew?.messageid)}
                          >
                             <span>
                                <img
                                   src="assets/images/icons/message-01.png"
                                   alt=""
                                />
                                <p>{messageNew?.text}</p>
                             </span>
                             ...
                             <span
                                onClick={() => Delete(messageNew?.messageid)}
                             >
                                <img
                  ...
                  : allNotifications?.map((messageNew) => (
                       <li key={messageNew?.messageid}>
                          <div
                             className={styles["list-item"]}
                             onClick={() => ReadMsg(messageNew?.messageid)}
